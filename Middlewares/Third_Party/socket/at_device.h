...
struct AT_Device {
    ...
    uint32_t resp_line_len[RESP_ROW_LEN];/*记录长度包含/0*/
    uint32_t resp_line_counts;/*记录行数*/
...
//int  esp8266_socket_alloc(SocketRole role, uint8_t *id);        /* 分配一个空闲 socket */
//int  esp8266_socket_free(uint8_t id);                           /* 释放 socket */
//struct socket_t *esp8266_socket_get(uint8_t id);                 /* 查看 socket 信息 */

///* ==================== WiFi / TCP 接口 ==================== */
//int esp8266_wifi_connect(char *ssid, char *password);
//int esp8266_tcp_connect2server(struct UART_Device *puart, uint8_t socket,
//                                uint16_t port, const char *server_ip);
//int esp8266_tcp_server(struct UART_Device *puart, uint16_t port);
//int esp8266_send_data(struct UART_Device *puart, uint8_t socket,
//                       uint8_t *data, uint16_t len, uint32_t timeout_ms);
//int esp8266_parse_ipd(struct UART_Device *puart, uint8_t *link_id,
//                       uint8_t *recv_data, uint16_t *data_len);
//int esp8266_close2(struct UART_Device *puart);
//int esp8266_test(struct UART_Device *puart);
...
// */
//ssize_t esp8266_send(int sockfd, const void *buf, size_t len, int flags);
